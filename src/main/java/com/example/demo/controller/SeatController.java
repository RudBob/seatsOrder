...
public class SeatController {
    ...
    public Msg cancelOrder() {
        seatService.cancelOrder();
        return Msg.success();
    }
    ...
    public Msg addTime(@RequestParam(value = "addHours") Integer addHours,
                       @RequestParam(value = "addMinutes", required = false, defaultValue = "0") Integer addMinutes) {
        boolean res = seatService.addTime(addHours, addMinutes);
        if (res) {
            return Msg.success();
        }
    ...
    public Msg getSeat(@RequestParam(value = "useHours") Integer hours,
        ...
        boolean res = seatService.getSeat(startDatetime, endDatetime);
        if (res) {
            return Msg.success();
        }
        return Msg.fail().setMsg("出现未知错误，请尽快联系管理员查看");
    ...
    public Msg outSeat() {
        boolean res = seatService.outSeat();
        if (res) {
            return Msg.success();
        }
    ...
    public Msg tempOut(@RequestParam(value = "outTimeMinutes", required = false, defaultValue = "15") Integer outTimeMinutes) {
        boolean res = seatService.tempOut(outTimeMinutes);
        if (res) {
            return Msg.success();
        }
    ...
    public Msg cancelTempOut() {
        boolean res = seatService.cancelTempOut();
        if (res) {
            return Msg.success();
        }
        return Msg.fail();
